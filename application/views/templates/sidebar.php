...
<ul class="navbar-nav bg-gradient-info sidebar sidebar-dark accordion" id="accordionSidebar">
    ...
    <a class="sidebar-brand d-flex align-items-center justify-content-center" href="javascript:;">
        ...
        <div class="sidebar-brand-text mx-3">SMK Al Furqon Bantarkawung</div>
    </a>
    ...
    <?php foreach ($menu as $m) : ?>
        ...
        <?php foreach ($subMenu as $sm) : 
            if ($sm['title'] == "Modul Siswa") {?>
                <?php foreach ($kelas as $k) : ?>
                    <?php if ($k['is_active'] == 1) {?>
                        <li class="nav-item <?= ($title == $sm['title']." ".$k['kelas']." ".$k['namakelas']) ? 'active' : ''; ?>">
                <!-- Nav Item - Dashboard -->
                            <a class="nav-link pb-0" href="<?= base_url($sm['url']."/".$k['kodekelas']);?>">
                                <i class="<?= $sm['icon']; ?>"></i>
                                <span><?= $sm['title'];?> <?= $k['kelas']; ?> <?= $k['namakelas']; ?> </span></a>
                        </li>                        
                <?php } endforeach;
            }else{
            ?>
